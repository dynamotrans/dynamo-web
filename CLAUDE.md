...
- **Todo mergeado a `main` y desplegado en producción** vía Vercel (commits del día en `main`: `1654bc5` → `f839ed3` → `9ddb964` → `9bfffc1` → `b86e5e9`). El workflow de uptime queda activo en background. Verificación visual del usuario en `dynamotrans.com` con hard-refresh si la caché muestra versión vieja
