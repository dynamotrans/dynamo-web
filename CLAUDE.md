...
- **Botones con esquinas rectas** (border-radius 50px → 0): primero los del hero `.btn-wa` y `.btn-email` (commit `290ba81`); después los CTA grandes `.btn-cta-wa` y `.btn-cta-out` usados en la barra "Cotiza Online" y la sección "¿Listo para empezar?" (commit `af3a9f1`)
- **Todo mergeado a `main` y desplegado** en producción (dynamotrans.com) vía Vercel. Verificación visual del hero oscuro y los botones rectos pendiente del usuario (producción bloquea peticiones automáticas de Claude). Nota recurrente: hard-refresh en móvil si la caché muestra la versión antigua
