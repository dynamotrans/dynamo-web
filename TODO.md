...
- [ ] **Refactor anti-duplicación del tarifador (Paso A)**: el formulario del tarifador está **copiado** en `index.html` (público) y `dashboard.html` (panel) como dos copias independientes → se desincronizan (causó el conflicto de merge del 2026-07-04). Extraer la lógica común a `js/tarifador.js` compartido, como ya se hizo con `js/festivos.js`. Ir con cuidado: verificar que público y panel se comportan idénticos antes/después (headless). **Paso B** posterior: extraer el "chrome" del portal (avatar FAB, selector de idioma, horarios/SCHEDULE, topbar) repetido en las 6 páginas a `js/portal-ui.js` + `css/portal.css`. Idealmente se resuelve solo al migrar a Next.js, pero el tarifador duplicado ya está costando merges hoy
