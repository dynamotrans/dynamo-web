...
- [ ] **Probar el email de Brevo en clientes reales**: `emails/prospeccion-brevo.html` está verificado solo en Chromium (700px y 380px). Antes de la primera campaña, mandar **email de prueba desde Brevo** a Gmail, Outlook (Windows, que es el que peor renderiza) y al móvil. Ojo a las ghost tables `<!--[if mso]-->` de las 3 zonas de 2 columnas (botones WhatsApp/Llamar, reseñas, tipos de camión)
- [ ] **Sincronizar `claude/sharp-dirac-E3UIO` con `main`** (regla 9): `main` lleva `emails/` e `images/email/` desde el 2026-08-17 y la rama del portal aún no. Es `git merge main` en preview + push, sin conflicto esperado (solo archivos nuevos). Pendiente de permiso para empujar a esa rama
- [ ] **Campo `CONTACTO` en Brevo** (opcional): atributo con el nombre de la persona, para que el email de prospección salude "Buenos días, Marta" en vez de un genérico — es lo que más sube la tasa de respuesta. La línea con el `{% if %}` ya está escrita en `emails/README.md` § 2, solo hay que crear el atributo y pegarla
- [ ] **Script de API de Brevo** (opcional): subir/actualizar la plantilla sin abrir el editor (`POST /v3/smtp/templates`, `PUT /v3/smtp/templates/{id}`, `POST /v3/smtp/email`). Requiere `BREVO_API_KEY` en variable de entorno, **nunca commiteada**. Detalles en `emails/README.md` § 7
...
- [ ] **Quitar defaults SEV→MAD del tarifador antes de producción**: el formulario de Nueva carga y Nuevo presupuesto pre-rellena Sevilla 41001 → Madrid 28001 con lat/lon hardcoded para iterar rápido en pruebas. Buscar `=== DEFAULTS DE PRUEBA (BORRAR EN PRODUCCIÓN) ===` en `dashboard.html` (2 sitios: init y reset). Sustituir por `origInput.value = ''` y eliminar las asignaciones de dataset.lat/lon. ~10 líneas
