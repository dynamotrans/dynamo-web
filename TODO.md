...
## 📋 Normal
- [ ] **Aviso de festivo AUTONÓMICO específico de la provincia de origen/destino** (decidido 2026-07-08: **solo avisar, NO bloquear**). Hoy el aviso de la caja ETA es a nivel de **país** (sobre-avisa: en una ruta Sevilla→Madrid te avisa también de festivos de otras comunidades). Mejora: filtrar por la **comunidad exacta** del punto de recogida/entrega.
  - Nager.Date ya devuelve `counties` (ISO 3166-2, p.ej. `ES-AN` Andalucía, `ES-CB` Cantabria) por cada festivo regional. Hoy `festivos.js` lo **descarta** (solo guarda el nombre) → habría que conservarlo.
  - Hace falta un mapa **provincia → código de comunidad** (50 provincias / 17 CCAA) y capturar la comunidad del origen/destino desde Nominatim (`address.state` / `ISO3166-2-lvl4`), que hoy no se guarda (solo `cc`, lat, lon).
  - Comportamiento: **solo aviso** ⚠ ("El origen (Sevilla) tiene festivo autonómico ese día — posible cierre en la recogida"), NUNCA bloquea (evita impedir una fecha válida si la API falla). El bloqueo sigue siendo solo findes + festivos NACIONALES.
  - ⚠ Solo llega a nivel **autonómico**, no municipal (la Feria de Sevilla u otros locales NO los cubre Nager). Probablemente se afina de verdad con el backend.
  - ⚠ `festivos.js` es archivo **compartido** (web pública + portal, byte-idéntico main/preview): cualquier cambio hay que sincronizarlo en las dos ramas.
